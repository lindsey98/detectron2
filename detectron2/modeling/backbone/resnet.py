...
__all__ = [
    ...
    "BasicBlock",
    "BottleneckBlock",
    "BottleneckBlockQuantize",
    "DeformBottleneckBlock",
    "BasicStem",
    "ResNet",
    "make_stage",
    "build_resnet_backbone",
    "build_resnet_backbone_quantize"
]
...
class BottleneckBlockQuantize(CNNBlockBase):
    """
    The quantized version of bottleneck residual block used by ResNet-50, 101 and 152
    defined in :paper:`ResNet`.  It contains 3 conv layers with kernels
    1x1, 3x3, 1x1, and a projection shortcut if needed.
    """
...
class BasicStemQuantize(CNNBlockBase):
    """
    The standard ResNet stem (layers before the first residual block).
    """
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        if depth in [18, 34]:
            ...
            if deform_on_per_stage[idx]:
                ...
                #                 stage_kargs["block_class"] = BottleneckBlock
                #                 else:
                stage_kargs["block_class"] = BottleneckBlockQuantize
        blocks = ResNet.make_stage(**stage_kargs)
        in_channels = out_channels
        out_channels *= 2
        bottleneck_channels *= 2
